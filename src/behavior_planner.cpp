...
EventType StateMachine_GetEvent(Vehicle vehicle, stateMachine_t *stateMachine){
  ...
  bool left_fwd_free = stateMachine->highway.fwd_gap[0] >= 30;
  bool mid_fwd_free = stateMachine->highway.fwd_gap[1] >= 30;
  bool right_fwd_free = stateMachine->highway.fwd_gap[2] >= 30;
  
  bool left_bwd_free = stateMachine->highway.backward_gap[0] >= 30;
  bool mid_bwd_free = stateMachine->highway.backward_gap[1] >= 30;
  bool right_bwd_free = stateMachine->highway.backward_gap[2] >= 30;
 
/*
  cout << "mid_fwd_free" << endl;
  cout << stateMachine->highway.forward_gap[1] << endl;
  
  cout << "mid_bwd_free" << endl;
  cout << stateMachine->highway.backward_gap[1] << endl;
  
  cout << "left_fwd_free" << endl;
  cout << stateMachine->highway.forward_gap[0] << endl;
  
  cout << "left_bwd_free" << endl;
  cout << stateMachine->highway.backward_gap[0] << endl;
  
  cout << "right_forward_free" << endl;
  cout << stateMachine->highway.forward_gap[2] << endl;
  
  cout << "right_backward_free" << endl;
  cout << stateMachine->highway.backward_gap[2] << endl;
...
*/
  ...
  if(count == 1){ //START
    ...
    event = EV2; //CRUISE
           ...
           (currBehavior == CRUISE && (myLane == 0 && (mid_fwd_free && mid_bwd_free && stateMachine->highway.fwd_gap[0]<stateMachine->highway.fwd_gap[1]))) ||\
           (currBehavior == CRUISE && (myLane == 2 && (mid_fwd_free && mid_bwd_free && stateMachine->highway.fwd_gap[2]<stateMachine->highway.fwd_gap[1])))\
           ){
    if (myLane == 0){stateMachine->lanechange_dir = 1;};
