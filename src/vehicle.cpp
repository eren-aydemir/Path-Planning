...
Vehicle::Vehicle(const int i){
  ...
  this->state.d = 6;
  this->state.target_speed = V_PLAN;
...
Vehicle::Vehicle(){
 ...
 if((this->state.d > LANE(0) - 2) && (this->state.d < LANE(0) + 2)){
   this->state.current_lane = 0; //LEFT

 }else if((this->state.d > LANE(1) - 2) && (this->state.d < LANE(1) + 2)){
   this->state.current_lane = 1; //MID

 }else if((this->state.d > LANE(2) - 2) && (this->state.d < LANE(2) + 2)){
